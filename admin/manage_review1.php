...
if(isset($_REQUEST['acti'])){
		...
		header("Location:manage_member.php?msg=delete");
	...
	if(isset($_REQUEST['actis'])){
		...
		header("Location:manage_member.php?msg=active");
	...
	if(isset($_REQUEST['inacti'])){
		...
		header("Location:manage_member.php?msg=inactive");
	...
	{
	$whereclause.= " where membername like '".$_REQUEST["keyword"]."%'";
	} 
	if(!$_REQUEST["alpha"]=="")
	{
	$whereclause.= " where membername like '".$_REQUEST["alpha"]."%'";
	}
//	$whereclause.= "where id like '%".$_REQUEST["keyword"]."%'";

$productselect = "SELECT * FROM `tbl_review` where memstatus=1";
...
function ifdel()
	...
	if(flag)

		{
			...
			{
document.manage_content.action= "manage_member.php?acti=delete"
	...
	if(flag)

		{
			...
			{
document.manage_content.action= "manage_member.php?actis=act"
				document.manage_content.submit();
	...
	if(flag)

		{
			...
			{
document.manage_content.action= "manage_member.php?inacti=inact"
				document.manage_content.submit();
...
function deletemember(pid)
{
	...
	if(confirm("Are you sure to Delete This Member?"))
	{
	FormName.action	= "manage_member.php?action=delete&pid="+pid;
	FormName.submit();
	}
}
</script>
  ...
  <div class="panel panel-default">
   ...
   <div class="row" style="margin-left:15px;margin-right:15px;">
	...
	$result = mysqli_query($db,$productselect);	
			...
			$k=1;
	while($cat_det = mysqli_fetch_array($result)){$i++;?>
                                    <tr>
                                        ...
                                        <td style="color:green"><?php if($cat_det["memstatus"]=='1'){?>
                                          Active 
                                          <?php } else {?>
                                          Inactive 
                                          <?php } ?>
