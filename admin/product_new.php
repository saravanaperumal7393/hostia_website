...
// $sql= "INSERT INTO tbl_new_product(product_name,product_type,mrp,product_image,product_category)VALUES(".mysqli_real_escape_string($db,$product_name).",".mysqli_real_escape_string($db,$product_type).",".mysqli_real_escape_string($db,$mrp).",".mysqli_real_escape_string($db,$upload_path).",".mysqli_real_escape_string($db,$product_category).")";
	$sql= "INSERT INTO tbl_new_product(product_name,product_type,mrp,product_image,product_category)VALUES('$product_name','$product_type','$mrp','$product_image','$product_category')";
// print_r($sql);die;
...
$upload_path =$_POST[img];
  ...
  // print_r( $_POST[product_category]);die;
	$sql = "UPDATE tbl_new_product SET product_image='".mysqli_real_escape_string($db,$upload_path)."',product_name='".$_POST[product_name]."',product_type='".$_POST[product_type]."',mrp='".$_POST[mrp]."',product_category='".$_POST[product_category]."' WHERE id =$_POST[id] ";
...
if ($_POST["Submit"]=="Delete")
   {
	$sql = "delete from tbl_new_product WHERE id=$_POST[id]";
                                    if (!mysqli_query($db,$sql))
                                                  { 
												  echo $delk;
												  die('Error:' . mysqli_error());
...
$cc= mysqli_query($db,"Select *from  tbl_new_product",$con); //check how many records exists in the database
...
<form class="form-horizontal bucket-form" action="product_new.php"  method="post" enctype="multipart/form-data" id="add_product" onSubmit="return validate();"  name="add_product">
...
Height: 1080px <br/>
			...
			echo "upload/nophoto.jpg";
                ...
                <option value="0">Select Type</option>
                 ...
                 <option value="10 Pieces">10 Pieces</option>
                    		 ...
                    		 <div class="col-sm-2">   <input type="text" name="mrp" id="mrp" class="form-control" value="<?php echo $_REQUEST[mrp]; ?>" />
                    		 	</div>
                           </div>
                                  <div class="form-group">
                    		  <label class="col-sm-3 control-label"> New Price </label>
                    		 <div class="col-sm-2">     <input type="text" name="product_category" id="product_category" class="form-control" value="<?php echo $_REQUEST[product_category]; ?>" />
                    		 	</div>
                           </div>
					...
					<div class="form-group">
						...
						<div class="form-group">
						<label class="col-xs-12 " style="text-align:center;"><input name="Submit22" type="reset" class="btn btn-danger" value="Back"  onclick="location.replace('product_new.php')" />
 ...
 { ?>
							...
							{
							?>
                                                      <input name="Submit2" type="reset" class="btn btn-danger" value="Add New" onClick="location.replace('product_new.php?"bid"=sav')" />
                                                      <?php 
 ...
 ?>
                                                      <input name="Submit" type="reset" class="btn btn-danger" value="Reset" onClick="location.replace('product_new.php?"bid"=sav')" /></label></div>
   ...
   <span class="style51"> <strong><br />
                                        ...
                                        <table width="99%" border="1" align="center" cellpadding="0" cellspacing="0" bordercolor="#3366CC" bgcolor="#999999" style="border: solid 1px;   border-color:#999999;">
                                          ...
                                          <?php 
  $result = mysqli_query($db,"SELECT * FROM tbl_new_product order by id DESC");
$i=1; 
  ...
  extract( $row );
			  ?>
                                          <tr>
                                            ...
                                            <br></td>
                                            <td width="6%" align="center" valign="middle" bgcolor="#FFFFFF"><span class="style26"><?php echo "<a href = 'product_new.php?bid=up&image=$product_image&id=$id&product_name=$product_name&product_type=$product_type&mrp=$mrp&product_category=$product_category' target=_parent>"; ?> <img src="images/up.png" alt="" /> <?php echo "</a>" ?></span> </td>
                                            <td width="11%" align="center" valign="middle" bgcolor="#FFFFFF" ><span class="style26"><?php echo "<a  href = 'product_new.php?bid=del&image=$product_image&id=$id&product_name=$product_name&product_type=$product_type&mrp=$mrp&product_category=$product_category' target=_parent>"; ?> <img src="images/del.png" alt="" /> <?php echo "</a>"; ?></span></td>
                                          </tr>
                                          <?php
