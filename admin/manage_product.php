...
if(isset($_REQUEST['delete_x'])){
		...
		foreach($_REQUEST['catid'] as $val){
			$res1 = mysqli_fetch_array(mysqli_query($db,"SELECT * FROM tbl_product WHERE product_id=".$val.""));
		unlink($res1["Product_image"]);
		unlink($res1["thumb_image"]);

			$sql = mysqli_query($db,"delete from tbl_product where product_id='$val'");

		}

		header("Location:manage_product.php?cat_id=".$_REQUEST["cat_id"]."&subcat_id=".$_REQUEST["subcat_id"]."&"."msg=delete&subcatname=".$_REQUEST["subcatname"]."&catname=".$_REQUEST["catname"]);
	...
	if(isset($_REQUEST['btn_active_x'])){
		...
		foreach($_REQUEST['catid'] as $val){
			...
			$sql = mysqli_query($db,"update tbl_product set Product_status='1' where product_id='$val'");

		}

		header("Location:manage_product.php?cat_id=".$_REQUEST["cat_id"]."&subcat_id=".$_REQUEST["subcat_id"]."&"."msg=active&subcatname=".$_REQUEST["subcatname"]."&catname=".$_REQUEST["catname"]);
	...
	if(isset($_REQUEST['btn_inactive_x'])){
		...
		header("Location:manage_product.php?cat_id=".$_REQUEST["cat_id"]."&subcat_id=".$_REQUEST["subcat_id"]."&"."msg=inactive&subcatname=".$_REQUEST["subcatname"]."&catname=".$_REQUEST["catname"]);
	...
	{
	$whereclause.= " and Product_name like '%".$_REQUEST["keyword"]."%'";
	} 
	if(isset($_REQUEST["action"]))
	{
		$res1 = mysqli_fetch_array(mysqli_query($db,"SELECT * FROM tbl_product WHERE product_id=".$_REQUEST["pid"].""));
		unlink($res1["Product_image"]);
		unlink($res1["thumb_image"]);
	$dele="delete from tbl_product where product_id='".$_REQUEST["pid"]."'";
	mysqli_query($db,$dele);
	
	}
$productselect = "select * from tbl_product where cat_id='".$_REQUEST["cat_id"]."' and subcat_id='".$_REQUEST["subcat_id"]."' $whereclause order by product_id desc";
$productselect = "select * from tbl_product where cat_id='".$_REQUEST["cat_id"]."' and subcat_id='".$_REQUEST["subcat_id"]."' $whereclause order by product_id desc";
...
function deletemember(pid)
{
	if(confirm("Are you sure to Delete this Record?"))
			...
			FormName		= document.manage_content;
				FormName.action	= "manage_product.php?action=delete&pid="+pid+"&cat_id="+<?php echo $_REQUEST["cat_id"];?>+"&subcat_id="+<?php echo $_REQUEST["subcat_id"];?>+"&subcatname="+<?php echo $_REQUEST["subcatname"];?>+"&catname="+<?php echo$_REQUEST["catname"];?>
	FormName.submit();
...
<form name="manage_content" method="post">
  ...
  <div class="panel panel-default">
    ...
    <div class="row w3-res-tb">
	  <div class="col-xs-12 col-sm-12 m-b-xs"><p style="text-align:center;font-size:20px;color:#82072e;"><?php echo $_REQUEST["subcatname"];?></p></div>
	<div class="col-xs-12 col-sm-9 m-b-xs">
       <a href="manage_subcategory.php?cat_id=<?php echo $_REQUEST["cat_id"];?>&catname=<?php echo $_REQUEST["catname"];?>">Manage Category</a> >> Manage Product
         
      </div>
      <div class="col-xs-12 col-sm-3 m-b-xs">
       <table align="right"  cellpadding="0" cellspacing="0" style="margin-bottom:10px;">
                              <tr>
                                  <td width="59" align="center"><a href="add_product.php?catid=<?php echo $_REQUEST["cat_id"];?>&subcatid=<?php echo $_REQUEST["subcat_id"];?>&subcatname=<?php echo $_REQUEST["subcatname"];?>&catname=<?php echo $_REQUEST["catname"];?>"><img src="images/add.png" height="50" border="0" style="margin-top:  -5px;"/></a></td>
	 ...
	 <?php if($_REQUEST["msg"]=="active"){?>
       <center>     <span class="style16">Sub Category Activated Successfully</span>
            <?php } elseif($_REQUEST["msg"]=="inactive"){?>
            <span class="style16">Sub Category Deactivated Successfully</span>
            <?php } elseif($_REQUEST["action"]=="delete"){?>
            <span class="style16">Sub Category Deleted Successfully</span></center>
            <?php } ?>
   <div class="row" style="margin-left:15px;margin-right:15px;">
	...
	$result = mysqli_query($db,$productselect);	
			...
			$records_per_page = 1000; // no of records per page
			$current_page_no = 1;
			if ($_REQUEST['page'])
			{
			...
			$pageL = new pageList();
			$pageL -> pageList($total_records, $records_per_page, $current_page_no);
			$pageL->generate();
			if($total_records !=0)
			{
	...
	?>
					<div class="col-xs-12 col-sm-6 col-md-4" style="text-align:center;margin-bottom:20px;">
						...
						<?php if($cat_det["Product_status"]=='1'){?>
                    <strong><h5 style="background-color:#92072e;padding:5px;color:#fff;">NEW ARRIVAL</h5></strong>
                    <?php } else {?><strong><h5 style="padding:5px;color:#fff;">&nbsp;</h5></strong><?php }?>
						...
						<div style="background: #eee5ff; padding: 5px;">
									...
									<h4 style="max-height:40px;min-height:40px;font-size:16px;color:red;"><?php echo $cat_det["Product_rate"];?> </h4>
									<p><a href="add_product.php?catid=<?php echo $_REQUEST["cat_id"];?>&subcatid=<?php echo $_REQUEST["subcat_id"];?>&pdt_id=<?php echo $cat_det["product_id"];?>&subcatname=<?php echo $_REQUEST["subcatname"];?>&catname=<?php echo $_REQUEST["catname"];?>"><img src="images/edit.png" width="16" height="16" border="0" alt=""><span class="style6">Edit</span></a>  <label style="float:right;">
