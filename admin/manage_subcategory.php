...
if(isset($_REQUEST['delete_x'])){
		...
		foreach($_REQUEST['catid'] as $val){
			...
			$sql = mysqli_query($db,"delete from tbl_subcategory where subcat_id='$val'");

		}

		header("Location:manage_subcategory.php?cat_id=".$_REQUEST["cat_id"]."&"."msg=delete&catname=". $_REQUEST["catname"]);
	...
	if(isset($_REQUEST['btn_active_x'])){
		...
		foreach($_REQUEST['catid'] as $val){
			...
			$sql = mysqli_query($db,"update tbl_subcategory set subcat_status='1' where subcat_id='$val'");

		}

		header("Location:manage_subcategory.php?cat_id=".$_REQUEST["cat_id"]."&"."msg=active&catname=". $_REQUEST["catname"]);
	...
	if(isset($_REQUEST['btn_inactive_x'])){
		...
		foreach($_REQUEST['catid'] as $val){
			...
			$sql = mysqli_query($db,"update tbl_subcategory set subcat_status='0' where subcat_id='$val'");

		}

		header("Location:manage_subcategory.php?cat_id=".$_REQUEST["cat_id"]."&"."msg=inactive&catname=". $_REQUEST["catname"]);

	}
	if(isset($_REQUEST["action"]))
	{
	$dele="delete from tbl_subcategory where subcat_id='".$_REQUEST["pid"]."'";
	...
	{
	$whereclause.= " and subcat_name like '%".$_REQUEST["keyword"]."%'";
	} 
$productselect = "select * from tbl_subcategory where cat_id='".$_REQUEST["cat_id"]."' $whereclause order by sortid";
$mp=1;
?>
<!DOCTYPE html>
...
function deletemember(pid)
{
  if(confirm("Are you sure to Delete this Record?"))
	...
	FormName.action	= "manage_subcategory.php?cat_id=<?php echo $_REQUEST["cat_id"];?>&action=delete&pid="+pid+"&catname=<?php echo $_REQUEST["catname"];?>";
	FormName.submit();
			}
}
</script>
...
<form name="manage_content" method="post">
  ...
  <div class="panel panel-default">
    ...
    <div class="row w3-res-tb">
	 <div class="col-xs-12 col-sm-12 m-b-xs"><p style="text-align:center;font-size:20px;color:#82072e;"><?php echo $_REQUEST["catname"];?></p></div>
	<div class="col-xs-12 col-sm-6 m-b-xs">
      ...
      <div class="col-xs-12 col-sm-6 m-b-xs">
       <table align="right"  cellpadding="0" cellspacing="0" style="margin-bottom:10px;">
                              <tr>
                                  <td width="59" align="center"><a href="add_subcategory.php?cat_id=<?php echo $_REQUEST["cat_id"];?>&catname=<?php echo $_REQUEST["catname"];?>"><img src="images/add.png" height="50" border="0" style="margin-top:  -5px;"/></a></td>
	 ...
	 <?php if($_REQUEST["msg"]=="active"){?>
       <center>     <span class="style16"> Category Activated Successfully</span>
            <?php } elseif($_REQUEST["msg"]=="inactive"){?>
            <span class="style16"> Category Deactivated Successfully</span>
            <?php } elseif($_REQUEST["action"]=="delete"){?>
            <span class="style16"> Category Deleted Successfully</span></center>
            <?php } ?>
	...
	while($cat_det = mysqli_fetch_array($productquery)){$i++;?>
          <tr>
            <td><input type="checkbox" name="catid[]" id="catid[]" value="<?php echo $cat_det['subcat_id'];?>" /></td>
            <td><a href="manage_product.php?cat_id=<?php echo $_REQUEST["cat_id"];?>&subcat_id=<?php echo $cat_det["subcat_id"];?>&subcatname=<?php echo $cat_det["subcat_name"];?>&catname=<?php echo $_GET["catname"];?>"style=" <?php if($cat_det["subcat_status"]=='1'){?> color:#993333;<?php } else{?>  color:#333333; <?php }?>font-weight:bold;"><?php echo $cat_det["subcat_name"];?></a></td>
            <td> <span class="text-ellipsis" style=" <?php if($cat_det["subcat_status"]=='1'){?> color:#993333;<?php } else{?>  color:#333333; <?php }?> font-weight:bold;"><?php if($cat_det["subcat_status"]=='1'){?>
                                    Active
                                    <?php } else {?>
                                    Inactive
                                    <?php } ?></span></td>
            ...
            <td>
             <a href="add_subcategory.php?cat_id=<?php echo $_REQUEST["cat_id"];?>&subcat_id=<?php echo $cat_det['subcat_id'];?>&catname=<?php echo $_GET["catname"];?>"><img src="images/edit.png" width="16" height="16" border="0" /></a> &nbsp; <a href="javascript:deletemember('<?php echo $cat_det['subcat_id'];?>');" class="style5"><img src="images/delete.png" width="20" height="20" border="0" /></a>
            </td>
          </tr>
     <?php }} ?>
                                <?php if($total_records<=0){?>
                                <tr>
                                  <td height="43" colspan="4" class="desccol"><div align="center"><font color="#FF0000">No Sub Category Added</font></div></td>
                                </tr>
                                <?php }?>
        </tbody>
      </table>
        ...
        <div class="col-sm-7 text-right text-center-xs">                
		   ...
		   else
		   {
			  ...
			  $pageL->currentPage;
		   	  $pageL->totalPages;
			  echo "Page ";
			  $pageL->displayLink("manage_subcategory.php?cat_id=".$_REQUEST["cat_id"], true, true, 1000); //display as  << < 1 2 3 4 5 6 7 > >> 
		   }
		   ?>
        </div>
      </div>
