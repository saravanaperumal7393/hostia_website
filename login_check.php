...
$date = date('Y-m-d H:i:s');
	...
	// echo $password1;
	// exit;
	$selquery="select * from members where email='$username1' and password='$password1'";
	// echo $selquery;
	// exit;
...
//mysqli_query($db,"update tbl_admin set admin_status=1 where admin_uname='$username1' and admin_pwd='$password1'");
		...
		$_SESSION["uname"]=$det["email"];
		
		if($_SESSION["uname"]== 'admin@hostiahosur.com'){
       
			// ?cat_id=1&subcat_id=1&subcatname=Gallery%20&catname=Gallery
			header("location:members/member_list.php?cat_id=1&uname=username1");
			ob_end_flush();
		}
		else{
			header("location:members/index.php?cat_id=1&uname=username1");
			
			ob_end_flush();
		}
		
		// header("location: members/add_post.php?cat_id=1&catname=PRODUCTS");
		// header("location:members/index.php?cat_id=1&email=username1");
		// ob_end_flush();
	} 	
	else
	{
	...
	header("location:login.php?a=Please Enter Correct Username Or Password");
	ob_end_flush();
	}
?>
<div align="center"><a href="../login.php" class="style1">Back</a></div>
<?php
ob_end_flush();
?>